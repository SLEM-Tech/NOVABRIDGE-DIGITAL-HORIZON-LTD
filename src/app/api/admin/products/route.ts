...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Ensure slug is unique by appending a timestamp if it already exists
    const existing = await queryOne(`SELECT id FROM ${T.products} WHERE slug = $1`, [finalSlug]);
    ...
    const product = await queryOne<any>(
      `INSERT INTO ${T.products}
       ...
       RETURNING *`,
      [name, uniqueSlug, sku, description, short_description, price, regular_price || price, sale_price, stock_status, stock_quantity, status, type],
    );
    ...
    if (categories.length > 0) {
      await query(
        `UPDATE ${T.categories} SET count = (
          SELECT COUNT(*) FROM ${T.productCategories} WHERE category_id = ${T.categories}.id
        ) WHERE id = ANY($1::int[])`,
        [categories],
      );
    }
    ...
    return NextResponse.json(
      { message: error?.message || "Server error", detail: error?.detail || null },
      { status: 500 },
    );
  }
}
