...
> The project uses **pnpm**. Run `npm install -g pnpm` first if you don't have it. Both `react-query` v3 (customer-facing) and `@tanstack/react-query` v5 (admin panel) are used simultaneously.
...
├── components/
│   ├── lib/
│   │   └── woocommerce.tsx   # Internal fetch-based API client (no WooCommerce SDK)
...
| Problem | Fix |
|---------|-----|
| Install fails with peer dep errors | Use `pnpm install` (project uses pnpm) |
